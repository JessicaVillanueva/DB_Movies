...
CREATE TABLE likes(
	id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
    criterio ENUM('like', 'dislike') NOT NULL,
    movie_id INT NOT NULL,
    CONSTRAINT FK_MOVIE_ID_MOVIE_lk FOREIGN KEY(movie_id)
    REFERENCES movies(id) ON DELETE CASCADE,
    user_id INT NOT NULL,
    CONSTRAINT FK_USER_ID_USER_lk FOREIGN KEY(user_id)
    REFERENCES users(id) ON DELETE CASCADE
);
...
-- Vista para mostrar likes/dislikes de una película
-- DROP VIEW V_MOVIE_LIKES_DISLIKES;
CREATE VIEW V_MOVIE_LIKES_DISLIKES AS
SELECT m.id, m.title AS "movie_title", CONCAT_WS(" ", l.criterio) AS "criterio", movie_id FROM movies m
INNER JOIN likes l ON l.movie_id = m.id;
SELECT * FROM V_MOVIE_LIKES_DISLIKES;

-- Procedimiento almacenado para dar like/dislike
DELIMITER $$
CREATE PROCEDURE LIKE_DISLIKE(IN criterio ENUM('like','dislike'), IN id_movie INT, IN id_user INT)
BEGIN
	INSERT INTO likes(criterio, movie_id, user_id) VALUES (criterio, id_movie, id_user);
END$$
DELIMITER ;
